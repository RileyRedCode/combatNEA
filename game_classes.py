...
def checkCollision(x1, x2, y1, y2, ox1, ox2, oy1, oy2):  # Give the start and end points of each objects x and y axis
    if ((x1 <= ox1 < x2) or (x1 < ox2 <= x2) or (ox1 < x1 < ox2)) and (
...
class World:
    ...
    def __init__(self, textMap, obstacles):
        ...
        self.img = pygame.image.fromstring(img_bytes, img_size,
                                           img_mode)  # fromstring requires the bytes for the image, the size and the mode
        self.tileMap = mapGen.textMap
        self.worldSurface = pygame.Surface((MAP_RES))
        self.worldSurface.blit(self.img, (0, 0))
        self.nodes = [[False for count in range(MAP_WIDTH * (SCREEN_SIZE[1] // TILE_SIZE))] for i in
                      range(MAP_WIDTH * (SCREEN_SIZE[0] // TILE_SIZE))]  # Establishes a plain grid
    ...
    def reAdjust(self):
        if self.owner.rect.center[0] != (SCREEN_SIZE[0] // 2) - 1:  # If not center
            if self.owner.mapX > (SCREEN_SIZE[0] // 2) - 1 and self.owner.mapX < (
                ...
                self.owner.mapX += self.owner.rect.center[0] - (
                            (SCREEN_SIZE[0] // 2) - 1)  # Adjusts the map coOrds if they are centered
            elif self.owner.mapX > MAP_RES[0] - SCREEN_SIZE[0]:  # Identifies that they are at the right edge
                self.owner.mapX = (MAP_RES[0] - SCREEN_SIZE[0]) + self.owner.rect.center[
                    0]  # Adjusts the map coOrds if they are at the right border
            ...
            if self.owner.mapX > (SCREEN_SIZE[0] // 2) - 1 and self.owner.mapX < (
                ...
                self.owner.rect.center = ((SCREEN_SIZE[0] // 2) - 1, self.owner.rect.center[
                    1])  # Only changes the rect x coOrdinate if not at border
        ...
        if self.owner.rect.center[1] != (SCREEN_SIZE[1] // 2) - 1:  # If not center
            if self.owner.mapY > (SCREEN_SIZE[1] // 2) - 1 and self.owner.mapY < (
                ...
                self.owner.mapY += self.owner.rect.center[1] - (
                            (SCREEN_SIZE[1] // 2) - 1)  # Adjusts the map coOrds if they are centered
            elif self.owner.mapY > MAP_RES[1] - SCREEN_SIZE[1]:  # Identifies that they are at the right edge
                self.owner.mapY = (MAP_RES[1] - SCREEN_SIZE[1]) + self.owner.rect.center[
                    1]  # Adjusts the map coOrds if they are at the right border
            ...
            if self.owner.mapY > (SCREEN_SIZE[1] // 2) - 1 and self.owner.mapY < (
                ...
                self.owner.rect.center = (self.owner.rect.center[0], (
                            SCREEN_SIZE[1] // 2) - 1)  # Only changes the rect x coOrdinate if not at border
    ...
    def update(self, enemyList, serverSide=False):
        ...
        for enemy in enemyList:
            if checkCollision(self.mapX - 2, self.mapX + 2, self.mapY - 2, self.mapY + 2,
                              enemy.mapX - (enemy.width // 2), enemy.mapX + (enemy.width // 2),
                              enemy.mapY - (enemy.height // 2), enemy.mapY + (enemy.height // 2)):
                if serverSide:
                    return True, enemy
...
class Character(pygame.sprite.Sprite):
    ...
    def __init__(self, x, y, conn=None):
        ...
        self.image_orig = pygame.image.load(
            "rocket.png")  # This one is always upright which is useful for rotating with movement
    ...
    def fire(self, coOrds, calculated=False):
        if calculated == False:
            ...
            for count in range(len(sides)):
                ...
                sides[count] = sides[
                                   count] / seconds  # divides the sides by the amount of seconds to work out how much the axis should increment per second
...
class Enemy(pygame.sprite.Sprite):
    ...
    def locate(self, playerList, world):  # could do with Nodes
        ...
        paths.enqueue([world.nodes[Y][X], 0, math.sqrt(
            ((world.nodes[Y][X].mapX - target.mapX) ** 2) + ((world.nodes[Y][X].mapY - target.mapY) ** 2)),
            ...
            for neighbour in current[0].neighbours:  # Evaluates all neighbours to be queued
                ...
                if neighbour in visited:
                    ...
                    for path in paths.queue:
                        if neighbour == path[0]:
                            ...
                            if path[1] > current[1] + abs(math.sqrt(
                                    ((neighbour.mapX - target.mapX) ** 2) + ((neighbour.mapY - target.mapY) ** 2))):
                                path[1] = current[1] + abs(math.sqrt(
                                    ((neighbour.mapX - target.mapX) ** 2) + ((neighbour.mapY - target.mapY) ** 2)))
                                path[2] = abs(math.sqrt(((neighbour.mapX - current[0].mapX) ** 2) + (
                                            (neighbour.mapY - current[0].mapY) ** 2))) + abs(math.sqrt(
                                    ((neighbour.mapX - target.mapX) ** 2) + ((neighbour.mapY - target.mapY) ** 2)))
                                pathNodes = current[3]
                ...
                if not present:
                    ...
                    if neighbour == goalNode:
                        ...
                        paths.enqueue([neighbour, abs(math.sqrt(
                            ((neighbour.mapX - current[0].mapX) ** 2) + ((neighbour.mapY - current[0].mapY) ** 2))) +
                                       current[1],
                                       # this calculates the hypotenuse from one neighbour to another then adds the previous nodes path cost
                                       abs(math.sqrt(((neighbour.mapX - current[0].mapX) ** 2) + (
                                                   (neighbour.mapY - current[0].mapY) ** 2))) + abs(math.sqrt(
                                           ((neighbour.mapX - target.mapX) ** 2) + (
                                                       (neighbour.mapY - target.mapY) ** 2))), pathNodes])
...
class ServerEnemy:
    ...
    def locate(self, playerList, serverNodes):
        ...
        if playerList:  # If a player has been sighted
            for player in playerList:  # This finds the closest one
                distance = (self.mapX - playerList[player]["location"][0]), (
                            self.mapY - playerList[player]["location"][1])
                hypotenuse = math.sqrt((distance[0] ** 2) + (distance[1] ** 2))
                if not shortestDis or hypotenuse < shortestDis:
        ...
        X = int(self.mapX // TILE_SIZE)  # Need the location of the enemy in reference to Nodes
        Y = int(self.mapY // TILE_SIZE)
        paths.enqueue([serverNodes[Y][X], 0, math.sqrt(((serverNodes[Y][X].mapX - target["location"][0]) ** 2) + (
                    (serverNodes[Y][X].mapY - target["location"][1]) ** 2)), []])  # This is the start node
        visited = []
        while not solution:
            current = paths.dequeue()
            visited.append(current[0])
                ...
                if neighbour in visited:
                    ...
                    for path in paths.queue:
                        if neighbour == path[0]:
                            present = True
                            # This updates a path if a shorter route to the same node is found
                            if path[1] > current[1] + abs(math.sqrt(((neighbour.mapX - target["location"][0]) ** 2) + (
                                    (neighbour.mapY - target["location"][1]) ** 2))):
                                path[1] = current[1] + abs(math.sqrt(((neighbour.mapX - target["location"][0]) ** 2) + (
                                            (neighbour.mapY - target["location"][1]) ** 2)))
                                path[2] = abs(math.sqrt(((neighbour.mapX - current[0].mapX) ** 2) + (
                                            (neighbour.mapY - current[0].mapY) ** 2))) + abs(math.sqrt(
                                    ((neighbour.mapX - target["location"][0]) ** 2) + (
                                                (neighbour.mapY - target["location"][1]) ** 2)))
                                pathNodes = current[3]
                                pathNodes.append(current[0])
                                path[3] = pathNodes
                        ...
                        paths.enqueue([neighbour, abs(math.sqrt(
                            ((neighbour.mapX - current[0].mapX) ** 2) + ((neighbour.mapY - current[0].mapY) ** 2))) +
                                       ...
                                       abs(math.sqrt(((neighbour.mapX - current[0].mapX) ** 2) + (
                                                   (neighbour.mapY - current[0].mapY) ** 2))) + abs(math.sqrt(
                                           ((neighbour.mapX - target["location"][0]) ** 2) + (
                                                       (neighbour.mapY - target["location"][1]) ** 2))), pathNodes])
            if current[0] == goalNode:
                solution = True
...
def nodeSetup(obstacleList, nodes):
    ...
    for y in range(MAP_WIDTH * (SCREEN_SIZE[1] // TILE_SIZE)):
        for x in range(MAP_WIDTH * (SCREEN_SIZE[0] // TILE_SIZE)):
            collision = False
            for obstacle in obstacleList:
                if checkCollision(x * 40, x * 40 + TILE_SIZE, y * 40, y * 40 + TILE_SIZE,
                                  obstacle.mapX - (obstacle.width // 2), obstacle.mapX + (obstacle.width // 2),
                                  obstacle.mapY - (obstacle.height // 2), obstacle.mapY + (obstacle.height // 2)):
                    collision = True
            if not collision:
